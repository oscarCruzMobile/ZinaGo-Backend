...
class ValidateAccountUseCase(
    ...
    fun execute(id: Long): UserResponse {
        ...
        // No validar si ya está validado
        if (user.lvalidated) {
            throw BusinessException("La cuenta del usuario $id ya está validada")
        }

        // Activar validación: lvalidated = true, validatedAt = ahora
        userRepository.validateAccount(
            id  = id,
